(define (domain domain_robot)
   ...
   (:action serveDrinkCold
    ...
    (:durative-action move
        ...
        :condition (and 
            (over all(waiter ?w)) 
            (at start (and (atRobot ?t1 ?w)))
        )
        :effect (and 
            (at end (not (atRobot ?t1 ?w)))
            (at end (atRobot ?t2 ?w))
        )
    )
    ...
    (:durative-action prepareCold
        ...
        :condition (and 
            (over all(waiter ?w))
            (over all(barista ?b))
            (at start (and (>(toMakeCold ?table)0)))
        )
        :effect (and 
            (at end 
                (and (atDrinkcold bar ?d) (decrease(toMakeCold ?table)1) (increase(toServeCold ?table)1))
            )
        )
    )
    ...
    (:durative-action prepareHot
        ...
        :condition (and 
            (over all(waiter ?w))
            (over all(barista ?b))
            (at start (and (>(toMakeHot )0)))
        )
        :effect (and 
            (at end (and (atDrinkhot bar ?d) (decrease(toMakeHot ?table)1) (increase(toServeHot ?table)1)))
        )
    )
    ...
    (:durative-action cleanTable
        ...
        :condition (and 
            ...
            (at start (and (dirtyTable ?t) (not (free ?t)) (free ?w)))
        )
        :effect (and 
            (at end (and (not (dirtyTable ?t)) (free ?t)))
        )
    )
